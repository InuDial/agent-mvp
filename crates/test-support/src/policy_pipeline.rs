...
pub struct TestPolicyPipeline<F: PolicyContextFactory> {
...
impl<F: PolicyContextFactory> Default for TestPolicyPipeline<F> {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl<F> PolicyEngine<F> for TestPolicyPipeline<F>
where
    F: PolicyContextFactory,
{
...
impl<F: PolicyContextFactory> TestPolicyPipeline<F> {
    pub fn new() -> Self {
        Self {
            next_policy_id: 1,
