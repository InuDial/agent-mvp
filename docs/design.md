...
| `ToolContext` | Gives tools controlled access facades and nested invocation without exposing backend handles. | `crates/kernel/src/tool/context.rs` |
...
| `ActionExecutor` | Keeps execution on backend/store objects while preserving a shared grant flow. | `crates/kernel/src/action.rs`, `crates/access-fs/src/backend.rs`, `crates/access-network/src/backend.rs`, `crates/access-monty/src/store.rs` |
| `PolicyEngine` | Trait for deciding actions; its default `grant` implementation creates `Granted<Action>`. | `crates/kernel/src/policy/traits.rs` |
| `PolicyPipeline` | Concrete app policy engine with inbound, typed, outbound, and default-deny evaluation. | `crates/app/src/policy_pipeline.rs` |
| Inbound policies | Apply global gates before resource-specific policy can allow anything. | `crates/app/src/policy_pipeline.rs` |
| `CapabilityEnvelopePolicy` | Prevents an action from exceeding the invocation's effective capabilities. | `crates/app/src/policy_pipeline.rs` |
| Typed policies | Keep resource authorization scoped to concrete action types. | `crates/kernel/src/policy/traits.rs`, `crates/access-fs/src/policy.rs`, `crates/access-network/src/policy.rs`, `crates/access-monty/src/policy.rs` |
...
| `policy.evaluate` audit | Shows each policy's decision at DEBUG level for diagnostics. | `crates/kernel/src/audit.rs`, `crates/kernel/src/policy/traits.rs` |
...
4. `mvp-app/src/policy_pipeline.rs`: policy evaluation order
